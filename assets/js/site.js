...
(function () {
  ...
  // ----- Activities Globe -----
  // The interactive globe now lives in its own page-scoped script,
  // assets/js/activity-globe.js, loaded only on the Activities page.
  ...
  var guestbookMessage = document.getElementById('guestbookMessage');
  var guestbookCount = document.getElementById('guestbookCount');
  var guestbookStatus = document.getElementById('guestbookStatus');

  if (guestbookMessage && guestbookCount) {
    guestbookMessage.addEventListener('input', function () {
      guestbookCount.textContent = this.value.length + ' / ' + this.maxLength;
    });
  }
  ...
  if (guestbookForm && guestbookStatus) {
    guestbookForm.addEventListener('submit', function (e) {
      ...
      var submitButton = guestbookForm.querySelector('button[type="submit"]');
      var submitLabel = submitButton.querySelector('span');
      var formData = new FormData(guestbookForm);
      var endpoint = guestbookForm.action.replace('formsubmit.co/', 'formsubmit.co/ajax/');

      submitButton.disabled = true;
      submitLabel.textContent = 'Sending…';
      guestbookStatus.className = 'guestbook-status';
      guestbookStatus.textContent = 'Sending your message securely…';
      ...
      fetch(endpoint, {
        ...
        .then(function (data) {
          ...
          guestbookForm.reset();
          if (guestbookCount) guestbookCount.textContent = '0 / 1200';
          guestbookStatus.className = 'guestbook-status is-success';
          guestbookStatus.textContent = 'Thank you — your message has been sent.';
        })
        .catch(function () {
          guestbookStatus.className = 'guestbook-status is-error';
          guestbookStatus.textContent = 'The message could not be sent. Please try again or use the email link above.';
        })
        .finally(function () {
